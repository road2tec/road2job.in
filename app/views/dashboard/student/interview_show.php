<?php
$isCompleted = $interviewSession['status'] === 'completed';
...
<h1 class="h4 mb-1">Interview</h1>
<p class="text-muted mb-4">Status: <span class="badge text-bg-<?= $isCompleted ? 'success' : 'secondary' ?>"><?= e(ucfirst(str_replace('_', ' ', $interviewSession['status']))) ?></span></p>
...
<?php if ($isCompleted): ?>
    ...
    <?php if ($score !== null): ?>
        <div class="card">
            <div class="card-header"><i class="bi bi-clipboard-check me-2 text-primary"></i>Employer Feedback</div>
            <div class="card-body">
                <div class="row g-3 mb-2">
                    <div class="col-md-4"><span class="small text-muted">Keyword Coverage</span><div class="h6"><?= (int) $score['keyword_score'] ?>/5</div></div>
                    <div class="col-md-4"><span class="small text-muted">Confidence</span><div class="h6"><?= (int) $score['confidence_score'] ?>/5</div></div>
                    <div class="col-md-4"><span class="small text-muted">Technical Accuracy</span><div class="h6"><?= (int) $score['technical_score'] ?>/5</div></div>
                </div>
                <?php if (!empty($score['notes'])): ?>
                    <p class="small mb-0"><strong>Notes:</strong> <?= nl2br(e($score['notes'])) ?></p>
                <?php endif; ?>
            </div>
        </div>
    <?php else: ?>
        <div class="alert alert-info mb-0">You've completed this interview. The employer hasn't submitted their feedback yet.</div>
    <?php endif; ?>
<?php else: ?>
    <div class="interview-session-recorder"
         data-finish-url="<?= url('/dashboard/my-interviews/' . $interviewSession['id'] . '/finish') ?>"
        ...
        <div data-state="done" hidden>
            <div class="card">
                <div class="card-header"><i class="bi bi-check-circle me-2 text-success"></i>Interview Completed</div>
                <div class="card-body">
