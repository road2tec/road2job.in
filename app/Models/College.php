...
{
    protected static string $table = 'colleges';
    ...
    public static function publicListing(?int $page = null, int $perPage = 24): array
    {
        if ($page === null) {
            $stmt = Database::connection()->prepare(
                "SELECT * FROM colleges WHERE name IS NOT NULL ORDER BY created_at DESC"
            );
            $stmt->execute();
        ...
        $countStmt = Database::connection()->prepare(
            "SELECT COUNT(*) FROM colleges WHERE name IS NOT NULL"
        );
        $countStmt->execute();
        $total = (int) $countStmt->fetchColumn();

        $stmt = Database::connection()->prepare(
            "SELECT * FROM colleges WHERE name IS NOT NULL ORDER BY created_at DESC LIMIT :limit OFFSET :offset"
        );
    ...
    public static function adminListing(?string $keyword = null, int $page = 1, int $perPage = 20): array
    {
        ...
        $countStmt = Database::connection()->prepare(
            "SELECT COUNT(*) FROM colleges c JOIN users u ON u.id = c.user_id WHERE {$where}"
        );
        ...
        $stmt = Database::connection()->prepare(
            "SELECT c.*, u.full_name AS owner_name, u.email AS owner_email, u.status AS owner_status
             FROM colleges c JOIN users u ON u.id = c.user_id
             WHERE {$where}
