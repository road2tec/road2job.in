...
class MockInterviewSessionQuestion extends Model
{
    protected static string $table = 'mock_interview_session_questions';
    ...
    public static function createForSession(int $sessionId, array $questions): void
    {
        ...
        foreach ($questions as $question) {
            static::insert([
                'mock_interview_session_id' => $sessionId,
    ...
    public static function forSession(int $sessionId): array
    {
        $stmt = Database::connection()->prepare(
            "SELECT sq.*, q.round_type, q.question_text, q.time_limit_seconds
             FROM mock_interview_session_questions sq
             JOIN interview_questions q ON q.id = sq.interview_question_id
             WHERE sq.mock_interview_session_id = :session_id
    ...
    public static function findForSession(int $sessionId, int $id): ?array
    {
        $stmt = Database::connection()->prepare(
            "SELECT sq.*, q.round_type, q.question_text, q.time_limit_seconds
             FROM mock_interview_session_questions sq
             JOIN interview_questions q ON q.id = sq.interview_question_id
             WHERE sq.id = :id AND sq.mock_interview_session_id = :session_id
             LIMIT 1"
        );
    ...
    {
        $stmt = Database::connection()->prepare(
            "SELECT COUNT(*) FROM mock_interview_session_questions WHERE mock_interview_session_id = :session_id AND answered_at IS NULL"
        );
        $stmt->execute(['session_id' => $sessionId]);
